...
class ReusableClassifier():
    ...
    def train(self,
        ...
        # Manual accuracy
        accuracy = (pred_labels == labels_test).sum()/len(labels_test) # or;
    ...
    def load(self, path:str):
        """Load model, scalar, and metadata from MODEL path"""
        model_path, _ = os.path.splitext(path)
        scaler_path = model_path + '_scaler.pkl'
        metadata_path = model_path + '.json'
        model_path = model_path + '.pkl'
...
if __name__ == '__main__':
    import duq_ds330.winequality
    wq = duq_ds330.winequality.WineQuality()
